    ...
    <main className="relative z-0 min-h-screen bg-[#09090D] px-5 py-8 pb-36 text-white sm:px-6">
      <div className="mx-auto max-w-md">
        <header className="mb-6">
          <p className="text-[11px] font-semibold uppercase tracking-[0.3em] text-[#8B7CF6]">FARM payouts</p>
          <h1 className="mt-2 text-3xl font-bold tracking-tight">Withdraw XLM</h1>
          <p className="mt-2 text-sm leading-6 text-slate-400">
            Request your mined balance to any Stellar public wallet and track every payout from one place.
          </p>
        </header>
          ...
          <div className="relative">
            <div className="flex items-end justify-between gap-4">
              <div>
                ...
                <p className="mt-2 font-mono text-4xl font-bold tabular-nums">{currentBalance.toFixed(6)}</p>
                <p className="mt-1 text-xs font-medium text-slate-500">XLM</p>
              </div>
              <div className={`rounded-2xl border px-3 py-2 text-right ${eligible ? "border-[#3B2A67] bg-[#19152A]" : "border-[#292935] bg-[#0F0F14]"}`}>
                <p className="text-[10px] uppercase tracking-wider text-slate-500">Status</p>
                <p className={`mt-1 text-sm font-semibold ${eligible ? "text-[#B9ACFF]" : "text-slate-400"}`}>
            ...
            <div className="mt-6">
              ...
              <div className="h-2 overflow-hidden rounded-full bg-[#24242D]">
                <div className="h-full rounded-full bg-[#8B5CF6] transition-all duration-500" style={{ width: `${progress}%` }} />
              </div>
              <div className="mt-2 flex justify-between text-xs text-slate-600">
                <span>Minimum</span>
                <span className="font-mono">{WITHDRAWAL_THRESHOLD.toFixed(6)} XLM</span>
              </div>
            </div>
          </div>
        </section>
        ...
        <section className="mt-5 rounded-2xl border border-[#22222D] bg-[#0F0F14] p-4">
          <div className="flex gap-3">
            <div className="mt-0.5 flex h-8 w-8 shrink-0 items-center justify-center rounded-full bg-[#1B1730] text-sm text-[#B9ACFF]">✦</div>
            <div>
              <p className="text-sm font-semibold">Use a Stellar public address</p>
              <p className="mt-1 text-xs leading-5 text-slate-500">
                Only enter a wallet address beginning with G. Never share a secret key or recovery phrase.
              </p>
            </div>
          </div>
        </section>
