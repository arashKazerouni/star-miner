    ...
    <main className="relative min-h-screen overflow-hidden bg-[#09090D] text-white">
      ...
      <header className="relative z-10 mx-auto flex max-w-6xl items-center justify-between gap-4 px-4 py-5 sm:px-6 sm:py-6 lg:px-8">
        <Link href="/" className="flex min-w-0 items-center gap-2.5 sm:gap-3">
          <div className="flex h-9 w-9 shrink-0 items-center justify-center rounded-xl bg-[#6C38FF] text-base font-black shadow-[0_0_30px_rgba(108,56,255,0.35)] sm:h-10 sm:w-10 sm:text-lg">
            F
          </div>
          <div className="min-w-0">
            <div className="truncate text-sm font-bold tracking-wide">Stellar Farm</div>
            <div className="hidden text-[10px] uppercase tracking-[0.28em] text-[#64748B] sm:block">
              FARM ecosystem
            </div>
          </div>
        </Link>
        <nav className="flex shrink-0 items-center gap-1 sm:gap-3">
          <Link
            ...
            className="rounded-lg px-3 py-2.5 text-sm font-medium text-[#94A3B8] transition hover:text-white sm:px-4"
          >
            Log in
          </Link>
          <Link
            href={`/register${referralQuery}`}
            className="rounded-lg bg-[#6C38FF] px-3.5 py-2.5 text-sm font-semibold shadow-[0_4px_20px_rgba(108,56,255,0.25)] transition hover:bg-[#5A2EE5] sm:px-4"
          >
            Get started
          </Link>
        </nav>
      ...
      <section className="relative z-10 mx-auto max-w-6xl px-4 pb-16 pt-10 text-center sm:px-6 sm:pb-20 sm:pt-16 lg:px-8 lg:pb-28 lg:pt-24">
        <div className="mx-auto inline-flex max-w-full items-center gap-2 rounded-full border border-[#6C38FF]/25 bg-[#6C38FF]/10 px-3 py-2 text-xs font-medium text-[#C4B5FD] backdrop-blur sm:px-3.5">
          <span className="h-1.5 w-1.5 shrink-0 rounded-full bg-[#10B981] shadow-[0_0_10px_rgba(16,185,129,0.8)]" />
          <span>Built by ❤️ for the Stellar ecosystem</span>
        </div>
        <h1 className="mx-auto mt-6 max-w-4xl text-4xl font-black leading-[1.05] tracking-[-0.04em] sm:mt-7 sm:text-6xl sm:leading-[1.02] lg:text-8xl">
          Grow with{" "}
          <span className="bg-gradient-to-r from-white via-[#C4B5FD] to-[#8B5CF6] bg-clip-text text-transparent">
            Stellar.
          </span>
        </h1>
        <p className="mx-auto mt-5 max-w-2xl px-1 text-sm leading-6 text-[#94A3B8] sm:mt-7 sm:px-0 sm:text-lg sm:leading-7">
        ...
        <div className="mx-auto mt-7 flex w-full max-w-md flex-col justify-center gap-3 sm:mt-9 sm:max-w-none sm:flex-row">
          <Link
            href={`/register${referralQuery}`}
            className="w-full rounded-xl bg-[#6C38FF] px-7 py-3.5 text-sm font-bold shadow-[0_8px_35px_rgba(108,56,255,0.3)] transition hover:-translate-y-0.5 hover:bg-[#5A2EE5] sm:w-auto sm:py-4"
          >
            Start your journey <span aria-hidden="true">→</span>
          </Link>
          <Link
            href={`/login${referralQuery}`}
            className="w-full rounded-xl border border-[#22222D] bg-[#121217]/80 px-7 py-3.5 text-sm font-semibold text-white backdrop-blur transition hover:border-[#353545] hover:bg-[#181820] sm:w-auto sm:py-4"
          >
            Access dashboard
          </Link>
        </div>
      ...
      <section className="relative z-10 mx-auto max-w-6xl px-4 pb-20 sm:px-6 sm:pb-24 lg:px-8">
        ...
        <div className="grid gap-4 md:grid-cols-3">
          {[
            ...
            <div
              key={item.icon}
              className="group rounded-2xl border border-[#22222D] bg-[#121217] p-5 transition hover:-translate-y-1 hover:border-[#6C38FF]/40 hover:shadow-[0_20px_60px_rgba(108,56,255,0.08)] sm:p-6"
            >
              <div className="flex h-11 w-11 items-center justify-center rounded-xl bg-[#6C38FF]/10 text-xs font-bold text-[#A78BFA]">
                {item.icon}
              </div>
              <h3 className="mt-5 text-lg font-bold sm:mt-6">{item.title}</h3>
      ...
      <section className="relative z-10 mx-auto max-w-6xl px-4 pb-20 sm:px-6 sm:pb-28 lg:px-8">
        <div className="overflow-hidden rounded-3xl border border-[#6C38FF]/20 bg-[radial-gradient(circle_at_80%_20%,rgba(108,56,255,0.22),transparent_45%),#121217] p-6 sm:p-8 md:p-12">
          <div className="max-w-2xl">
            ...
            <div className="mt-6 flex flex-wrap gap-2.5 text-xs text-[#64748B] sm:mt-7 sm:gap-3">
      ...
      <footer className="relative z-10 border-t border-[#22222D] px-4 py-7 sm:px-6 sm:py-8 lg:px-8">
        <div className="mx-auto flex max-w-6xl flex-col gap-3 text-center text-xs leading-5 text-[#64748B] sm:flex-row sm:items-center sm:justify-between sm:text-left">
